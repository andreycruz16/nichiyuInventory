...
$pdf->SetTitle('Service_ItemReport_'.$partNumber.'_'.date('M-d-y'));
       ...
       ORDER BY tbl_item_history.history_id ASC;";
        ...
        if (mysqli_num_rows($result) > 0) {
            ...
            while($row = mysqli_fetch_array($result, MYSQL_NUM)) { 
                ...
                // if($receivingReport == 'N/A') $receivingReport = '-';
                $transferType = $row[5];
                $quantity = $row[6];
                $username = $row[7];
                $customerName = $row[8];
...
$pdf->Output('Service_ItemReport_'.$partNumber.'_'.date('M-d-y').'.pdf', 'I');
